...
contract MultiRewardsDistributionRecipient {
  ...
  modifier onlyRewardsDistribution() {
    require(msg.sender == rewardsDistribution, "Caller is not MultiRewardsDistribution contract");
    _;
  }
}

contract StakingMultiRewards is IStakingMultiRewards, MultiRewardsDistributionRecipient, ReentrancyGuard {
  using SafeMath for uint256;
  ...
  function getRewardsPerTokenStored() external view returns  (uint256[] memory) {
  ...
  function getUserRewardPerTokenPaid(address user) external view returns  (uint256[] memory) {
  ...
  function rewardsPerToken() public view returns (uint256[] memory) {
    ...
    for (uint i = 0; i < rewardsPerTokenStored.length; i++) {
  ...
  function earned(address account) public view returns (uint256[] memory) {
    ...
    for (uint i = 0; i < _rewards.length; i++) {
      ...
      earnings[i] = balance.mul(_rewards[i].sub(paid)).div(1e18).add(reward);
    }
    return earnings;
  }
  ...
  function stakeWithPermit(uint256 amount, uint deadline, uint8 v, bytes32 r, bytes32 s) external nonReentrant updateRewards(msg.sender) {
  ...
  function notifyRewardAmount(uint256[] calldata addedRewards) external onlyRewardsDistribution updateRewards(address(0)) {
    require(rewardsTokens.length == addedRewards.length, "Added rewards and rewards length not equal");

    for (uint i = 0; i < addedRewards.length; i++) {
      if (block.timestamp >= periodFinish) {
        rewardRates[i] = addedRewards[i].div(rewardsDuration);
      } else {
        uint256 remaining = periodFinish.sub(block.timestamp);
        uint256 leftover = remaining.mul(rewardRates[i]);
        rewardRates[i] = addedRewards[i].add(leftover).div(rewardsDuration);
      }
      ...
      uint balance = rewardsTokens[i].balanceOf(address(this));
