...
struct Config {
    uint256 gasLimit;
    uint256 reserveProfitRatio;
    uint256 gasProfitMultiplier;
...
contract FlashSwapper is KeeperCompatibleInterface {
    ...
    AggregatorV3Interface public fastGasFeed; // in wei
    AggregatorV3Interface public wethPriceFeed; // in USD
    AggregatorV3Interface public rewardTokenPriceFeed; // in USD
    ...
    event FeeToChanged(address indexed _feeTo);
    ...
    constructor(
        ...
        config.reserveProfitRatio = _reserveProfitRatio;
        config.gasProfitMultiplier = 10;
        config.minBlockInterval = 1;
    ...
    function setReserveProfitRatio(uint256 _reserveProfitRatio) public {
        require(msg.sender == manager, "FORBIDDEN");
        config.reserveProfitRatio = _reserveProfitRatio;
    }

    function setGasProfitMultiplier(uint16 _gasProfitMultiplier) public {
        require(msg.sender == manager, "FORBIDDEN");
        config.gasProfitMultiplier = _gasProfitMultiplier;
    }

    function setMinBlockInterval(uint16 _minBlockInterval) public {
        require(msg.sender == manager, "FORBIDDEN");
        config.minBlockInterval = _minBlockInterval;
    }

    function setFastGasFeed(address _fastGasFeed) public {
        require(msg.sender == owner, "FORBIDDEN");
        fastGasFeed = AggregatorV3Interface(_fastGasFeed);
    }

    function setWethPriceFeed(address _wethPriceFeed) public {
        require(msg.sender == owner, "FORBIDDEN");
        wethPriceFeed = AggregatorV3Interface(_wethPriceFeed);
    }

    function setRewardTokenPriceFeed(address _rewardTokenPriceFeed) public {
        require(msg.sender == owner, "FORBIDDEN");
        rewardTokenPriceFeed = AggregatorV3Interface(_rewardTokenPriceFeed);
    }
        ...
        )
    {
        ...
        if(block.number % config.minBlockInterval == 0) {
            if(rewardToken != WETH) {
        ...
        returns (uint256)
    {
        ...
        if(address(fastGasFeed) != address(0)) {
            ...
            if(gasPrice > 0) {
                minProfitInWeth += config.gasProfitMultiplier
                                                    .mul(uint256(gasPrice))
                                                    .mul(config.gasLimit)
                                                    .mul(1e8);
            } 
        }
