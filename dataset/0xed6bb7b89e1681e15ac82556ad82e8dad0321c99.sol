...
interface INFTLocker {
    function locked(address user) external returns (uint256);
}
...
contract UpDown is OwnableUpgradeable, ReentrancyGuard {
    ...
    // Instance of TINY token (collateral currency for bet)
    IERC20 public tiny;
    address public nft; // TinyHero
    address public stats; // Tiny Hero Stats
    ...
    // SETTING
    uint256 public HOUSE_EDGE = 35; // 2%
    uint256 public MINIMUM_BET_AMOUNT = 500 ether;
    ...
    function initialize(address _tiny, address _nft, address _stats, address _nftLocker,  address _randomNumberGenerator, uint256[4] memory _bonusReward) external initializer {
        tiny = IERC20(_tiny);
        nft = _nft;
        stats = _stats;
        randomNumberGenerator = IRandomNumberGenerator(_randomNumberGenerator);
        nftLocker = INFTLocker(_nftLocker);
    ...
    function getBoostedIndex(uint256 _tokenId) public view returns (uint256 _index) {
        if (_tokenId > 0) {
            (, ITinyHeroStats.TinyAttribute memory attribute) =  ITinyHeroStats(stats).details(_tokenId);
            if (attribute.Productivity >= 11) _index = 1; // 30%
            else if (attribute.Productivity >= 6) _index = 2; // 2%
            else _index = 3; //
        } else {
            _index = 0;
        }
    ...
    {
        _bal = tiny.balanceOf(address(this)) - subAmount - balanceOfBet;
    }
    ...
    returns (uint256)
    {
        ...
        uint256 prize = 1000 ether;
        if (bal >= 10000000 ether) prize = 5000000 ether;
        else if (bal >= 5000000 ether) prize = 200000 ether;
        else if (bal >= 2000000 ether) prize = 100000 ether;
        else if (bal >= 1000000 ether) prize = 50000 ether;
        else if (bal >= 500000 ether) prize = 20000 ether;
        else if (bal >= 200000 ether) prize = 10000 ether;
        else prize = 5000 ether;
    ...
    */
    ...
    function placeBet(
        ...
        uint256 userStakedNft = nftLocker.locked(msg.sender);

        // Transfers the required tiny to this contract
        tiny.safeTransferFrom(msg.sender, address(this), amountBet);

        uint256 indexBoosted = getBoostedIndex(nftLocker.locked(msg.sender));
    ...
    function refundBet(address add) external onlyOwner {
        ...
        uint256 userStakedNft = nftLocker.locked(add);
        uint256 indexBoosted = getBoostedIndex(userStakedNft);
        uint256 winAmount = calculateWinAmount(bet.number, bet.isOver, bet.amount, indexBoosted);

        tiny.safeTransfer(add, bet.amount);
    ...
    function _settleBet(
        ...
        if (referralContract != address(0x0)) {
            ...
            if (ref != address(0x0)) {
                ...
                tiny.safeTransfer(referralContract, commission);
        ...
        if (checkWin(bet.number, bet.isOver, luckyNum)) {
            ...
            tiny.safeTransfer(bet.player, winAmount);
    ...
    function setNFTLocker(address _nftLocker) external onlyOwner {
        require(_nftLocker != address(0), "Contracts cannot be 0 address");
        nftLocker = INFTLocker(_nftLocker);
    }
    ...
    function setMinBet(uint256 value) external onlyOwner {
        require(value >= 500 ether && value <= 10000 ether);
    ...
    function setTiny(address _tiny) external onlyOwner {
        tiny = IERC20(_tiny);
    }
    ...
    function withdraw(uint256 amount) external onlyOwner {
        require(
            amount + lockBalanceForGame <= tiny.balanceOf(address(this)),
            "over available balance"
        );
        tiny.safeTransfer(owner(), amount);
    }
